...
public class ChallengeManager : MonoBehaviour
{
    ...
    [SerializeField]
    protected ChallengeStatusEnum challengeStatus;

    [Range(0, 120)] [SerializeField] private int maxTime;
    ...
    private void ShowScoreboard()
    {
        ...
        foreach (var score in _playerScoreList.scoreList)
        {
            ...
            scoreboard.text +=
                $"<pos=0%>{score.player}</pos><pos=45%>{score.points}</pos><pos=75%>{timeFormat}</pos>\n";
        }
    }
    ...
    private void WriteScoreOnFile()
    {
        try
        {
            System.IO.File.WriteAllText($"{Application.persistentDataPath}\\{file}",
                JsonUtility.ToJson(_playerScoreList));
        }
        catch (Exception e)
        {
